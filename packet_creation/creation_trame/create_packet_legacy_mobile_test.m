function full_packets = create_packet_legacy_mobile_test(fragments, primary_channel_index, advA_bi, multiple_send, data_type, name_flag, name, discoverable_flag, SIG, notif)
    ...
    for fragment_number=1:nb_different_packet % Total number of cells (different packets)
        ...
        for nb_rep = 1:multiple_send

            rep_number_bi = my_de2bi(nb_rep,16,"right-msb");
            rep_number_bi = [rep_number_bi(9:16) rep_number_bi(1:8)];
            % We fill in advData with information on the current package
            adData{fragment_number}(nb_rep,:)= [data_type_mobile ... % Type of file sent
                                              fragment_number_bi ... % packet n°
                                              fragment_number_max_bi ... % number max packet
                                              rep_number_bi ... % Times this packet has been repeated
                                              fragments{fragment_number}];  

            user_data_length = length(adData{fragment_number})/8 + 3;

            advData{fragment_number}(nb_rep,:) = [my_de2bi(user_data_length, 8, "right-msb") my_de2bi(0xFF, 8, "right-msb") SIG_bi adData{fragment_number}(nb_rep,:)]; %ad structure de base en manufacturer data

            if strcmp(discoverable_flag, "Y") == 1
                advData_disc{fragment_number}(nb_rep,:) = [my_de2bi(0x02, 8, "right-msb") my_de2bi(0x01, 8, "right-msb") my_de2bi(0x06, 8, "right-msb") advData{fragment_number}(nb_rep,:)];
            else
                advData_disc{fragment_number}(nb_rep,:) = advData{fragment_number}(nb_rep,:);
            end

            if strcmp(name_flag, "Y") == 1
                advData_name{fragment_number}(nb_rep,:) = [my_de2bi(local_name_length + 1, 8, "right-msb") ... % Longueur totale
                    my_de2bi(0x09, 8, "right-msb") ... % Type: Complete Local Name
                    local_name_bi ...
                    advData_disc{fragment_number}(nb_rep,:)]; % Nom en binaire
            else
                advData_name{fragment_number}(nb_rep,:) = advData_disc{fragment_number}(nb_rep,:);
            end

            if strcmp(notif, "Y") == 1
                advData_notif{fragment_number}(nb_rep,:) = [notif_length notif_type notif_UUID advData_name{fragment_number}(nb_rep,:)];
            else
                advData_notif{fragment_number}(nb_rep,:) = advData_name{fragment_number}(nb_rep,:);
            end

            payload{fragment_number}(nb_rep,:) = [advA_bi advData_notif{fragment_number}(nb_rep,:)]; 

            % Compute payload length for header
            length_payload = my_de2bi(length(payload{fragment_number}(nb_rep,:))/8,8,"right-msb");

            pdu{fragment_number}(nb_rep,:) = [header_legacy length_payload payload{fragment_number}(nb_rep,:)]; % PDU with header  

            pdu_crc{fragment_number}(nb_rep,:) = add_crc(pdu{fragment_number}(nb_rep,:).',default_CRC_init).';

            pdu_whitened{fragment_number}(nb_rep,:) = whitening_ble(pdu_crc{fragment_number}(nb_rep,:).',primary_channel_index).'; % CRC and Whitening on PDU
            
            full_packets{fragment_number}(nb_rep,:) = [preamble access_address_bi pdu_whitened{fragment_number}(nb_rep,:)];
